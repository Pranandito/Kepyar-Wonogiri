    ...
    <div class="d-flex flex-column align-items-center justify-content-center h-70">
        ...
        <div class="col-12">
            <div class="card-group">
                ...
                <!-- Tegangan Baterai Card-->
                <x-tegangan-card>{{ $pjuData->first()->arus ?? 0}}</x-tegangan-card>
            </div>
        </div>
...
<script>
    ...
    function updateGrafik() {
        fetch('{{ route('pju.getDayaHarianWeeklyData7') }}')
            ...
            .then(data => {
                ...
                if (chart) {
                    ...
                    chart.data.datasets[0].data = dayaHarian;
                    chart.update(); // Perbarui grafik
                } else {
                    // Membuat grafik jika belum ada
                    chart = new Chart(ctx, {
                        type: 'line',
                        data: {
                            labels: labels,
    ...
    function updateData() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let soc = 0;
                    if (voltage <= 10.66288) {
                        soc = 20.18264 * (voltage - 9.7879) / 0.610499835;
                    } else if (voltage >= 10.9911) {
                        soc = (49.85635 * (voltage - 10.9911) / 1.66889888) + 50.14361;
                    } else {
                        soc = (29.96096 * (voltage - 10.40029) / 0.5908053) + 20.18264;
                    }
                    
                    soc = Math.max(0, Math.min(soc, 100)); // Pastikan SOC dalam rentang 0% - 100%
                    soc = soc.toFixed(2); // Membulatkan SOC ke 2 desimal
                    // Update tegangan dan SOC
                    $('#tegangan').text(voltage + ' V - ' + soc + '%');
                },
        ...
        function updateIcon() {
            $.ajax({
                ...
                dataType: 'json', 
                success: function(data) {
                    let icon_daya = data.jadwal == 1 ? "bi bi-lightbulb" : "bi bi-sun-fill";
                    $('#icon-daya').attr("class", icon_daya);
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.error("Failed to fetch data: " + textStatus, errorThrown);
                }
        ...
        setInterval(updateGrafik, 120000);
